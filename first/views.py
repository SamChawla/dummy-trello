...
def updated_data(user_id):
    """
    Method to get a dictionary with keys as tags and values as tasks.
    """
    Tasks = TaskOptions.objects.filter(author_id=user_id)  # As in ToDo,InProgress etc.
    data = {}
...
class NewTaskView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            task_user = CustomUser.objects.get(id=task_user_id)
            ...
            data = updated_data(task_user_id)
        return render(request, 'first/tasks.html', {'data': data})
...
class NewTagForm(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            tag_user = CustomUser.objects.get(id=user_id)
            data = {'title': tag_title, 'order_no': order, 'author': tag_user}
